...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && strlen($_REQUEST['save']) > 0 && check_bitrix_sessid())
{
    ...
    if($_POST['FILENAME'] !== '' && $_POST['FILENAME'] !== '.php'){
        ...
        $oldfile = $_SERVER['DOCUMENT_ROOT'].$_POST['FILEDIR'].'/'.$_POST['OLDFILENAME'];
        $file = $_SERVER['DOCUMENT_ROOT'].$_POST['FILEDIR'].'/'.$_POST['FILENAME'];

        if($file !== $oldfile){
            $isSaved = rename($oldfile, $file);
        }
        if($_POST['FILECONTENT'] !== "" && $_POST['FILECONTENT'] !== $_POST['OLDCONTENT']){
...
?>
    <form method='post' action='' name='bootstrap'>
        ...
        <input type="hidden" name="FILECONTENT" value="<?=htmlspecialchars($content)?>" id="fileContent">
        <input type="hidden" name="OLDCONTENT" id="oldContent" value="<?=htmlspecialchars($content)?>">
        <? $tabControl->Begin();
